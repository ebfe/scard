...
const (
	ErrSuccess                = scardError(C.SCARD_S_SUCCESS)
	ErrInternalError          = scardError(C.SCARD_F_INTERNAL_ERROR)
	ErrCancelled              = scardError(C.SCARD_E_CANCELLED)
	ErrInvalidHandle          = scardError(C.SCARD_E_INVALID_HANDLE)
	ErrInvalidParameter       = scardError(C.SCARD_E_INVALID_PARAMETER)
	ErrInvalidTarget          = scardError(C.SCARD_E_INVALID_TARGET)
	ErrNoMemory               = scardError(C.SCARD_E_NO_MEMORY)
	ErrWaitedTooLong          = scardError(C.SCARD_F_WAITED_TOO_LONG)
	ErrInsufficientBuffer     = scardError(C.SCARD_E_INSUFFICIENT_BUFFER)
	ErrUnknownReader          = scardError(C.SCARD_E_UNKNOWN_READER)
	ErrTimeout                = scardError(C.SCARD_E_TIMEOUT)
	ErrSharingViolation       = scardError(C.SCARD_E_SHARING_VIOLATION)
	ErrNoSmartcard            = scardError(C.SCARD_E_NO_SMARTCARD)
	ErrUnknownCard            = scardError(C.SCARD_E_UNKNOWN_CARD)
	ErrCantDispose            = scardError(C.SCARD_E_CANT_DISPOSE)
	ErrProtoMismatch          = scardError(C.SCARD_E_PROTO_MISMATCH)
	ErrNotReady               = scardError(C.SCARD_E_NOT_READY)
	ErrInvalidValue           = scardError(C.SCARD_E_INVALID_VALUE)
	ErrSystemCancelled        = scardError(C.SCARD_E_SYSTEM_CANCELLED)
	ErrCommError              = scardError(C.SCARD_F_COMM_ERROR)
	ErrUnknownError           = scardError(C.SCARD_F_UNKNOWN_ERROR)
	ErrInvalidAtr             = scardError(C.SCARD_E_INVALID_ATR)
	ErrNotTransacted          = scardError(C.SCARD_E_NOT_TRANSACTED)
	ErrReaderUnavailable      = scardError(C.SCARD_E_READER_UNAVAILABLE)
	ErrShutdown               = scardError(C.SCARD_P_SHUTDOWN)
	ErrPciTooSmall            = scardError(C.SCARD_E_PCI_TOO_SMALL)
	ErrReaderUnsupported      = scardError(C.SCARD_E_READER_UNSUPPORTED)
	ErrDuplicateReader        = scardError(C.SCARD_E_DUPLICATE_READER)
	ErrCardUnsupported        = scardError(C.SCARD_E_CARD_UNSUPPORTED)
	ErrNoService              = scardError(C.SCARD_E_NO_SERVICE)
	ErrServiceStopped         = scardError(C.SCARD_E_SERVICE_STOPPED)
	ErrUnexpected             = scardError(C.SCARD_E_UNEXPECTED)
	ErrUnsupportedFeature     = scardError(C.SCARD_E_UNSUPPORTED_FEATURE)
	ErrIccInstallation        = scardError(C.SCARD_E_ICC_INSTALLATION)
	ErrIccCreateorder         = scardError(C.SCARD_E_ICC_CREATEORDER)
	ErrFileNotFound           = scardError(C.SCARD_E_FILE_NOT_FOUND)
	ErrNoDir                  = scardError(C.SCARD_E_NO_DIR)
	ErrNoFile                 = scardError(C.SCARD_E_NO_FILE)
	ErrNoAccess               = scardError(C.SCARD_E_NO_ACCESS)
	ErrWriteTooMany           = scardError(C.SCARD_E_WRITE_TOO_MANY)
	ErrBadSeek                = scardError(C.SCARD_E_BAD_SEEK)
	ErrInvalidChv             = scardError(C.SCARD_E_INVALID_CHV)
	ErrUnknownResMng          = scardError(C.SCARD_E_UNKNOWN_RES_MNG)
	ErrNoSuchCertificate      = scardError(C.SCARD_E_NO_SUCH_CERTIFICATE)
	ErrCertificateUnavailable = scardError(C.SCARD_E_CERTIFICATE_UNAVAILABLE)
	ErrNoReadersAvailable     = scardError(C.SCARD_E_NO_READERS_AVAILABLE)
	ErrCommDataLost           = scardError(C.SCARD_E_COMM_DATA_LOST)
	ErrNoKeyContainer         = scardError(C.SCARD_E_NO_KEY_CONTAINER)
	ErrServerTooBusy          = scardError(C.SCARD_E_SERVER_TOO_BUSY)
	ErrUnsupportedCard        = scardError(C.SCARD_W_UNSUPPORTED_CARD)
	ErrUnresponsiveCard       = scardError(C.SCARD_W_UNRESPONSIVE_CARD)
	ErrUnpoweredCard          = scardError(C.SCARD_W_UNPOWERED_CARD)
	ErrResetCard              = scardError(C.SCARD_W_RESET_CARD)
	ErrRemovedCard            = scardError(C.SCARD_W_REMOVED_CARD)
	ErrSecurityViolation      = scardError(C.SCARD_W_SECURITY_VIOLATION)
	ErrWrongChv               = scardError(C.SCARD_W_WRONG_CHV)
	ErrChvBlocked             = scardError(C.SCARD_W_CHV_BLOCKED)
	ErrEof                    = scardError(C.SCARD_W_EOF)
	ErrCancelledByUser        = scardError(C.SCARD_W_CANCELLED_BY_USER)
	ErrCardNotAuthenticated   = scardError(C.SCARD_W_CARD_NOT_AUTHENTICATED)
)
